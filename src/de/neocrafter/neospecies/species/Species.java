...
import de.neocrafter.neospecies.NeoSpecies;
...
public abstract class Species implements Listener, Player
{
	...
	public static Species getPlayer(Player player)
	{
		...
		NeoSQL database = plugin.getNeoDatabase();
		NeoStatement statement = database.query("SELECT * FROM `NeoSpecies_Player` WHERE Playername='" + player.getName() + "'");
		...
		{
			e.printStackTrace();
			return null; //Might want to return a Human as fail protection?
		}
		
		playerMap.put(player, ret);
		return ret;
	}
